...
import { CalendarRange, Plus, Check, ArrowRight, X, GraduationCap } from 'lucide-react';
...
function PromoteDrawer({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const submit = async () => {
    const items = rows.filter((r) => r.fromClassId && r.fromSectionId && r.toClassId && r.toSectionId);
    if (items.length === 0 && graduate.length === 0) { toast.error('Add at least one promotion or graduation'); return; }
    try {
      const res = await promote({ items, graduateClassIds: graduate }).unwrap();
      ...
      toast.error(e?.data?.error?.message || 'Could not promote');
    }
  };
    ...
    <Sheet open={open} onOpenChange={(o) => !o && onClose()}>
      <SheetContent side="right" hideClose className="w-full bg-card text-card-foreground sm:w-[520px]">
        <div className="flex h-full flex-col">
          ...
          <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-4">
            <SheetClose asChild><Button type="button" variant="secondary">Cancel</Button></SheetClose>
            <Button onClick={submit} loading={isLoading}>Promote</Button>
          </div>
        </div>
      </SheetContent>
    </Sheet>
